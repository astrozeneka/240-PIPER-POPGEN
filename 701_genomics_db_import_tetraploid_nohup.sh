...
SAMPLE_LIST="bam_list_tetraploid.txt"
...
while read -r bam; do
  ...
  GVCF=$(find "${PROJECT_ROOT}/results" -maxdepth 2 -path "*haplotype_caller_tetraploid*" -iname "${SLUG}.g.vcf.gz" | head -n1)
  [[ -z "${GVCF}" ]] && { echo "ERROR: GVCF not found for ${SLUG} (looked under ${PROJECT_ROOT}/results)"; exit 1; }
  V_ARGS+=("-V" "${GVCF}")
done < "${SAMPLE_LIST}"

OUTDIR="results/701_genomics_db_import_tetraploid"
...
OUT_LOG="logs/${CHR}_701_gdb_$$.out"
ERR_LOG="logs/${CHR}_701_gdb_$$.err"
